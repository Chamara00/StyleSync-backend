        ...
        if(!salonId){
            ...
            if (!staffIdOfSalon) {
                ...
                for (let i = 0; i < staffIdOfSalon.length; i++) {
                    ...
                    returnResuls.push(...findBlocks);
                } 
                return res.status(200).json({ status: 200, data:returnResuls,message: 'successfully display an  appointment.'}); 
        }
        }
